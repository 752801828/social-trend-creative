...
- `/acquire`：原始热点模块，可查看原始热点池并启动新获取。
- `/trends`：热点池模块，可查看拆分分类结果并对只有原始热点的轮次执行分类。
- `/prompts`：提示词池模块，可查看提示词全文、使用次数，并对已有热点池的轮次追加随机提示词。
...
- 管理页已拆成总览和四个独立 URL 模块，可按轮次查看原始热点池、热点池、提示词池和生图池；已移除旧的“勾选热点直接生图”。
