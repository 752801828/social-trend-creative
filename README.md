...
“获取热点”固定在同一任务中执行前三阶段；“一键完整流水线”继续执行到第四阶段。自动化设置可分别指定每天多个热点获取时间和随机生图时间，使用 24 小时制 `HH:MM` 并以逗号分隔，例如 `09:00,14:00,20:00`；两个开关默认关闭，定时生图会选择最新可用提示词任务。
...
各池卡片统一按内容创建日期倒序排列，不需要先选择任务；点击卡片会打开所属任务详情。模块间切换复用同一页面会话中的管理密钥，浏览器仍不持久化密钥。
