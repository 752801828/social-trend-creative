...
| ⑥ 图案生成 | 按等级配额从提示词池抽取的图案提示词 | Flow 返回后由 Pillow 解码，清除边缘连通背景并强制保存为真正带 alpha 通道的 `.png`；无法形成透明通道的结果拒绝入库 | 图案资产 `pattern_assets` |
...
所有新图案都经过 Pillow 实际图片处理：Flow 返回 JPG 或 PNG 后统一重新编码为 `.png`，清除与画布边缘连通的背景并检测 alpha 通道。透明检测失败会自动进行一次背景清理重试，仍无透明通道则拒绝保存，不会把 JPG 冒充透明 PNG。图案图库提供透明状态筛选和 PNG 下载按钮。
