...
独立的海外社媒热点图案创作服务：Gemini提取热点中的事件、梗、情绪、审美和视觉符号，Flow直接生成可印在杯子、手机壳、衣服、罩类、贴纸和海报等载体上的原创图案。项目不导入或修改 Flow2API、Gemini2API 源码，只通过HTTP API连接。

## 当前工作流

1. 每日定时或手动启动热点发现。
2. Gemini按最近时间窗口从所选来源平台寻找事件、梗、短语、情绪、审美、社群、季节节点和视觉符号。
3. 第二次Gemini请求将每条热点整理成独立、可复用的印刷图案方向，仅排除重复、空内容、明确不安全或依赖商标、版权角色、真人肖像的候选。
4. 证据URL和发布时间作为可选参考保存；缺失、无法解析或过期都不阻止生图，也不再设置“最终最多保留数”。
5. 管理页主流程会为全部 `ready` 候选直接调用Flow生成图案；图案本身不包含杯子、手机壳、衣服等商品样机，定时任务是否自动生图仍由独立开关控制。
...
- `POST /api/runs/full`：发现社媒热点，并为全部可用候选直接生成可印刷图案。
